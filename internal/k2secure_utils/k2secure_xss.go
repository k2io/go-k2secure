...
type processedDataStruct struct {
	value []byte
	next  *processedDataStruct
}

var doubleSemicolonPattern = regexp.MustCompile(DOUBLE_SEMICOLON)
...
func IsXSS(combinedData string) []string {
	var masterConstructList []string
	masterConstructList = getXSSConstructs(combinedData)
	return masterConstructList
}

func decodeRequestData(rq *models.Info_req) []string {
	var processedData []string
	for k, v := range rq.HeaderMap {
	...
	vVal := processURLEncodedDataForXSS(rq.Url)
	processedData = append(processedData, vVal...)
...
func decodeResponseData(rq *models.Info_req) []string {
	var processedData []string
...
func CheckForReflectedXSS(req *models.Info_req) string {
